...
** Added the include of utf_cfe_time.h to utf_cfe.h and added the utf_cfe_time_api.o specification
*to each
** example's makefile
** Revision 1.1 2008/04/17 08:09:01EDT ruperera
** Initial revision
** Member added to project
*c:/MKSDATA/MKS-REPOSITORY/MKS-CFE-PROJECT/tools/utf/examples/test12/project.pj
...
int main(void)
{
    ...
    UTF_put_text(
        "CFE_TIME_GetSTCF Default Return Value, (should be 0, 0), Seconds %x, Subseconds %x\n",
    ...
    UTF_put_text(
        "CFE_TIME_GetTime Return Value, (should be same as TAI), Seconds %x, Subseconds %x\n",
        system_time.Seconds, system_time.Subseconds);
    ...
    UTF_put_text("CFE_TIME_GetUTC Return Value, UTC = MET + STCF - Leap Seconds, Seconds %x, "
                 "Subseconds %x\n",
                 system_time.Seconds, system_time.Subseconds);
    ...
    UTF_put_text("CFE_TIME_CFE2FSSeconds Return Value, (should be time + CFE_TIME_FS_FACTOR = "
                 "789004802), %d\n",
                 CFE_TIME_CFE2FSSeconds(2));

    /* test File System seconds to cFE seconds */
    UTF_put_text(
        "CFE_TIME_FS2CFESeconds Return Value, (should be time - CFE_TIME_FS_FACTOR = 2) %x\n",
    ...
    UTF_TIME_set_function_hook(CFE_TIME_REGISTERSYNCHCALLBACK_HOOK,
                               (void*)&CFE_TIME_RegisterSynchCallbackHook);
    ...
    UTF_TIME_set_function_hook(CFE_TIME_UNREGISTERSYNCHCALLBACK_HOOK,
                               (void*)&CFE_TIME_UnregisterSynchCallbackHook);
