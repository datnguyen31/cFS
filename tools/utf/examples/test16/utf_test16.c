...
*c:/MKSDATA/MKS-REPOSITORY/MKS-CFE-PROJECT/tools/utf/examples/test16/project.pj
...
int32 CFE_TBL_RegisterHook(CFE_TBL_Handle_t* TblHandlePtr, const char* Name, uint32 Size,
...
int32 CFE_TBL_LoadHook(CFE_TBL_Handle_t TblHandle, CFE_TBL_SrcEnum_t SrcType,
...
int main(void)
{
    ...
    returnval =
        CFE_TBL_Register(&CriticalTblHandle, CriticalTableName, 10, CFE_TBL_OPT_CRITICAL, 0);
    UTF_put_text("* Return value is %x.  Expected 0.*\n", returnval);
    UTF_CFE_TBL_DisplayTableRegistryContents();
    ...
    UTF_CFE_TBL_DisplayTableRegistryContents();
    CFE_TBL_LoadWorkingBuffer(TblHandle1, CFE_TBL_SRC_ADDRESS, &Load1);
    ...
    SetLocalAppId(OwnerApp1Id);
    CFE_TBL_LoadWorkingBuffer(TblHandle1, CFE_TBL_SRC_ADDRESS, &Load2);
    ...
    UTF_put_text("CFE_TBL_Modified called for '%s' returned %x\n", TableName4,
                 CFE_TBL_Modified(TblHandle4));
    UTF_CFE_TBL_DisplayTableRegistryContents();
    UTF_put_text("CFE_TBL_Modified called for '%s' returned %x\n", CriticalTableName,
                 CFE_TBL_Modified(CriticalTblHandle));
    UTF_CFE_TBL_DisplayTableRegistryContents();
