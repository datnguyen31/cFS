...
*c:/MKSDATA/MKS-REPOSITORY/MKS-CFE-PROJECT/tools/utf/examples/test14/project.pj
...
int32 CFE_ES_CreateChildTaskHook(uint32* TaskIdPtr, const char* TaskName,
...
int main(void)
{
    ...
    UTF_put_text("CFE_ES_GetAppName(11) returned %x. AppName arg returned <%s>. Expected 0 and "
                 "<Application#1>.\n",
                 ReturnVal, AppName);
    ReturnVal = CFE_ES_GetAppName(AppName, 12, 128);
    UTF_put_text("CFE_ES_GetAppName(12) returned %x. AppName arg returned <%s>. Expected 0 and "
                 "<Application#2>.\n",
                 ReturnVal, AppName);
    ReturnVal = CFE_ES_GetAppName(AppName, 13, 128);
    UTF_put_text("CFE_ES_GetAppName(13) returned %x. AppName arg returned <%s>. Expected 0 and "
                 "<Application#3>.\n",
                 ReturnVal, AppName);
    ...
    strcat(VeryLongString,
           "will have to be truncated because only 128 characters can be accommodated ");
    ...
    /*    CFE_ES_CDSBlockDesc.CRC = CFE_ES_CalculateCRC(DataToWrite, CFE_ES_CDSBlockDesc.SizeUsed,
    ...
    ReturnVal =
        CFE_ES_RegisterCDS(&test14Data.myCDSHandle, sizeof(test14_TaskData_t), "Test14Data");
    ...
    if (ReturnVal == OS_SUCCESS)
    {
        UTF_put_text("Copied data of <%d>; <%s>; <%d> to CDS\n", test14Data.myCDSData.TestData1,
                     test14Data.myCDSData.TestData2, test14Data.myCDSData.TestData3);
        ...
        {
            UTF_put_text(
                "Restored data = <%d>; <%s>; <%d> to CDS\n", test14RestoredData.myCDSData.TestData1,
                test14RestoredData.myCDSData.TestData2, test14RestoredData.myCDSData.TestData3);
        }
    }
