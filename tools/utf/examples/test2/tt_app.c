...
*c:/MKSDATA/MKS-REPOSITORY/MKS-CFE-PROJECT/tools/utf/examples/test2/project.pj
** Revision 1.3 2007/02/01 13:18:17EST wfmoleski
** Needed to add the full path specification to all filenames to make these examples work with
*cFE 4.0
...
void TT_AppMain(void)
{
    ...
    */
    ...
    while (TRUE)
    {
        ...
        */
        Status = CFE_SB_RcvMsg(&TT_AppData.MsgPtr, TT_AppData.CmdPipe, 4);
...
void TT_AppInit(void)
{
    ...
    */
    CFE_EVS_Register(TT_AppData.EventFilters, TT_EVT_COUNT, CFE_EVS_BINARY_FILTER);

    /*
    ** Initialize housekeeping packet (clear user data area)...
    */
    CFE_SB_InitMsg(&TT_AppData.HkPacket, CFE_TT_HK_TLM_MID, sizeof(TT_HkPacket_t), TRUE);

    /*
    ** Create Software Bus message pipe...
    */
    CFE_SB_CreatePipe(&TT_AppData.CmdPipe, TT_AppData.PipeDepth, TT_AppData.PipeName);
    ...
    */
    Status = CFE_TBL_Register(&TT_AppData.TblHandle, "FourNumbers", 4, /* size of table in bytes*/
                              CFE_TBL_OPT_DBL_BUFFER | CFE_TBL_OPT_LOAD_DUMP, NULL);
    UTF_put_text("CFE_TBL_Register returned %lx\n", Status);

    Status = CFE_TBL_Load(TT_AppData.TblHandle, CFE_TBL_SRC_FILE, "/ram/tt_table_values0.dat");
    ...
    UTF_put_text("The initial values are:%d %d %d %d\n", *(int8*)TblPointer,
                 *(int8*)(TblPointer + 1), *(int8*)(TblPointer + 2), *(int8*)(TblPointer + 3));
    ...
    */
    CFE_EVS_SendEvent(TT_INIT_INF_EID, CFE_EVS_INFORMATION, "TT: Application Initialized");
            ...
            switch (CommandCode)
            {
                ...
                default:
                    CFE_EVS_SendEvent(TT_CC1_ERR_EID, CFE_EVS_ERROR,
                                      "Invalid ground command code: ID = 0x%X, CC = %d", MessageID,
                                      CommandCode);
                    break;
            }
            break;
...
void TT_NoopCmd(CFE_SB_MsgPtr_t msg)
{
    ...
    */
    if (TT_VerifyCmdLength(msg, ExpectedLength))
    {
        ...
        CFE_EVS_SendEvent(TT_NOOP_INF_EID, CFE_EVS_INFORMATION, "TT: No-op command");
    }
...
void TT_ResetCmd(CFE_SB_MsgPtr_t msg)
{
    ...
    */
    if (TT_VerifyCmdLength(msg, ExpectedLength))
    {
        ...
        CFE_EVS_SendEvent(TT_RESET_INF_EID, CFE_EVS_INFORMATION, "TT: Reset Counters command");
    }
    ...
    */
    if (ExpectedLength != ActualLength)
    {
        ...
        CFE_EVS_SendEvent(TT_LEN_ERR_EID, CFE_EVS_ERROR,
                          "TT: Invalid cmd pkt: ID = 0x%X,  CC = %d, Len = %d", MessageID,
                          CommandCode, ActualLength);
        result = FALSE;
        TT_AppData.ErrCount++;
    }
