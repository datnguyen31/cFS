...
*c:/MKSDATA/MKS-REPOSITORY/MKS-CFE-PROJECT/fsw/cfe-core/src/time/project.pj
** Revision 1.9 2007/03/07 10:54:24EST njyanchik
** I added a IsToneGood bit to the information that CFE_TIME_GetClockInfo function returns. The
*decison on a 'good' tone is made in the 1Hz ISR. If the tone is about
...
** I made a new API called CFE_TIME_GetClockInfo that is functionally equivilant to
*CFE_TIME_GetStateFlags. The reason I made a new function is because this function
...
**  will need to add 1 to the result. This will prevent the drift. Also, added in a check that
*prevents
...
UTF_TIME_HookTable_t UTF_TIME_HookTable = {NULL, NULL, NULL, NULL};

/* Return Code Stuff */
int32 cfe_time_api_return_value[NUM_OF_CFE_TIME_API_PROCS] = {UTF_CFE_USE_DEFAULT_RETURN_CODE,
...
{
    if (Index == CFE_TIME_GETMETSECONDS_HOOK)
    {
        UTF_TIME_HookTable.CFE_TIME_GetMETseconds = FunPtr;
    }
    else if (Index == CFE_TIME_GETMETSUBSECS_HOOK)
    {
        UTF_TIME_HookTable.CFE_TIME_GetMETsubsecs = FunPtr;
    }
    else if (Index == CFE_TIME_REGISTERSYNCHCALLBACK_HOOK)
    {
        UTF_TIME_HookTable.CFE_TIME_RegisterSynchCallback = FunPtr;
    }
    else if (Index == CFE_TIME_UNREGISTERSYNCHCALLBACK_HOOK)
    {
        UTF_TIME_HookTable.CFE_TIME_UnregisterSynchCallback = FunPtr;
    }
    else
    {
        UTF_error("Invalid TIME Hook Index In Set Hook Call %d", Index);
    }
}
...
int32 UTF_CFE_TIME_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_CFE_TIME_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_TIME_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        UTF_error("UTF Error: Read %d args w/script cmd UTF_SCRIPT_TIME_Set_Api_Return_Code. "
                  "Expected 2.\n",
                  argc - 1);
        UTF_exit();
    }
...
int32 CFE_TIME_RegisterSynchCallback(CFE_TIME_SynchCallbackPtr_t CallbackFuncPtr)
{
    ...
    /* Handle Preset Return Code */
    if (cfe_time_api_return_value[CFE_TIME_REGISTERSYNCHCALLBACK_PROC] !=
        UTF_CFE_USE_DEFAULT_RETURN_CODE)
    {
...
int32 CFE_TIME_UnregisterSynchCallback(CFE_TIME_SynchCallbackPtr_t CallbackFuncPtr)
{
    ...
    /* Handle Preset Return Code */
    if (cfe_time_api_return_value[CFE_TIME_UNREGISTERSYNCHCALLBACK_PROC] !=
        UTF_CFE_USE_DEFAULT_RETURN_CODE)
    {
