...
**          filesystem implementation for POSIX.  The real functions are contained in the file
*osfileapi.c.
**
** References:
**
...
** The UTF implementation did not have the return code setting code implemented in OS_lseek. This
*code  was added.
...
** Updated to contain the OS_FD _Table structure and the code to populate and remove entries from
*it.
...
UTF_OSFILEAPI_HookTable_t UTF_OSFILEAPI_HookTable = {NULL, NULL, NULL, NULL, NULL, NULL};
...
int32 UTF_OSFILEAPI_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_OSFILEAPI_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_OSFILEAPI_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        UTF_error("UTF Error: Read %d args w/script cmd UTF_SCRIPT_OSFILEAPI_Set_Api_Return_Code. "
                  "Expected 2.\n",
                  argc - 1);
        UTF_exit();
    }
...
{
    if (Index == OS_CREAT_HOOK)
    {
        UTF_OSFILEAPI_HookTable.OS_creat = FunPtr;
    }
    else if (Index == OS_WRITE_HOOK)
    {
        UTF_OSFILEAPI_HookTable.OS_write = FunPtr;
    }
    else if (Index == OS_READ_HOOK)
    {
        UTF_OSFILEAPI_HookTable.OS_read = FunPtr;
    }
    else if (Index == OS_OPENDIR_HOOK)
    {
        UTF_OSFILEAPI_HookTable.OS_opendir = FunPtr;
    }
    else if (Index == OS_READDIR_HOOK)
    {
        UTF_OSFILEAPI_HookTable.OS_readdir = FunPtr;
    }
    else if (Index == OS_CLOSE_HOOK)
    {
        UTF_OSFILEAPI_HookTable.OS_close = FunPtr;
    }
    else
    {
        UTF_error("Invalid OSFILEAPI Hook Index In Set Hook Call %d", Index);
    }
}
