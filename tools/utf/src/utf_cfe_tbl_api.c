...
** call was needed to simulate the fsw effects of a table load. So, this call was added back in with
*a
...
** Added the CFE_TBL_GetInfo implementation and modifications to reflect the fsw code changes. This
*included ES_GetBufPool argument changes.
...
** Extended capability to revert api calls to default return codes from a procedure call in the test
*driver.  Also expanded capability to cover 3 newest TBL api calls.
...
UTF_TBL_HookTable_t UTF_TBL_HookTable = {NULL, NULL, NULL, NULL, NULL};
/*
** Local Data
*/
...
{
    if (Index == CFE_TBL_REGISTER_HOOK)
    {
        UTF_TBL_HookTable.CFE_TBL_Register = FunPtr;
    }
    else if (Index == CFE_TBL_LOAD_HOOK)
    {
        UTF_TBL_HookTable.CFE_TBL_Load = FunPtr;
    }
    else if (Index == CFE_TBL_MANAGE_HOOK)
    {
        UTF_TBL_HookTable.CFE_TBL_Manage = FunPtr;
    }
    else if (Index == CFE_TBL_GETADDRESS_HOOK)
    {
        UTF_TBL_HookTable.CFE_TBL_GetAddress = FunPtr;
    }
    else if (Index == CFE_TBL_GETADDRESSES_HOOK)
    {
        UTF_TBL_HookTable.CFE_TBL_GetAddresses = FunPtr;
    }
    else
    {
        UTF_error("Invalid TBL Hook Index In Set Hook Call %d", Index);
    }
}
...
********************************************************************/
int32 CFE_TBL_Register(CFE_TBL_Handle_t* TblHandlePtr, const char* Name, uint32 Size,
                       uint16 TblOptionFlags, CFE_TBL_CallbackFuncPtr_t TblValidationFuncPtr)
{
    ...
    if (UTF_TBL_HookTable.CFE_TBL_Register)
        return (UTF_TBL_HookTable.CFE_TBL_Register(TblHandlePtr, Name, Size, TblOptionFlags,
                                                   TblValidationFuncPtr));
    ...
    if (Status == CFE_SUCCESS)
    {
        ...
        *TblHandlePtr = CFE_TBL_BAD_TABLE_HANDLE;
        ...
        else
        {
            ...
            /* User defined table addresses are only legal for single buffered, dump-only,
             * non-critical tables */
            if ((TblOptionFlags & CFE_TBL_OPT_USR_DEF_MSK) == CFE_TBL_OPT_USR_DEF_ADDR)
            {
                    ...
                    ((TblOptionFlags & CFE_TBL_OPT_CRITICAL_MSK) == CFE_TBL_OPT_CRITICAL))
                {
                    ...
                    CFE_ES_WriteToSysLog("CFE_TBL:Register-User Def tbl '%s' cannot be dbl buff, "
                                         "load/dump or critical\n",
                                         Name);
                }
            }
            else if ((TblOptionFlags & CFE_TBL_OPT_LD_DMP_MSK) == CFE_TBL_OPT_DUMP_ONLY)
            {
                ...
                if ((TblOptionFlags & CFE_TBL_OPT_BUFFER_MSK) == CFE_TBL_OPT_DBL_BUFFER)
                {
                    ...
                    CFE_ES_WriteToSysLog(
                        "CFE_TBL:Register-Dump Only tbl '%s' cannot be double buffered\n", Name);
                }
            }
        }
    }
        ...
        if (RegIndx != CFE_TBL_NOT_FOUND)
        {
            ...
            { /* same owner */
                ...
                { /* new size */
                    ...
                    CFE_ES_WriteToSysLog("CFE_TBL:Register-Attempt to register existing table "
                                         "('%s') with different size(%d!=%d)\n",
            ...
            else /* Duplicate named table owned by another Application */
            {
                ...
                CFE_ES_WriteToSysLog(
                    "CFE_TBL:Register-App(%d) Registering Duplicate Table '%s' owned by App(%d)\n",
        ...
        if ((Status & CFE_SEVERITY_BITMASK) != CFE_SEVERITY_ERROR)
        {
            ...
            if ((TblOptionFlags & CFE_TBL_OPT_USR_DEF_MSK) != CFE_TBL_OPT_USR_DEF_ADDR)
            {
                ...
                Status = CFE_ES_GetPoolBuf((uint32**)&RegRecPtr->Buffers[0].BufferPtr,
                                           CFE_TBL_TaskData.Buf.PoolHdl, Size);
                if (Status < 0)
                {
                    CFE_ES_WriteToSysLog("CFE_TBL:Register-1st Buf Alloc GetPool fail Stat=0x%08X "
                                         "MemPoolHndl=0x%08X\n",
                   ...
                   /* This makes code simpler and is transparent to tester */
            {
                ...
                if (Status < 0)
                {
                    CFE_ES_WriteToSysLog("CFE_TBL:Register-2nd Buf Alloc GetPool fail Stat=0x%08X "
                                         "MemPoolHndl=0x%08X\n",
                                         Status, CFE_TBL_TaskData.Buf.PoolHdl);
                }
                else
            ...
            {
                /* Register a CDS under the table name and determine if the table already exists
                 * there */
                ...
                if (Status == CFE_ES_CDS_ALREADY_EXISTS)
                {
                     ...
                     * TRUE); */
                    Status =
                        CFE_ES_RestoreFromCDS(WorkingBufferPtr->BufferPtr, RegRecPtr->CDSHandle);
                    ...
                    if (Status != CFE_SUCCESS)
                    {
                        ...
                        CFE_ES_WriteToSysLog("CFE_TBL:Register-Failed to recover '%s.%s' from CDS "
                                             "(ErrCode=0x%08X)\n",
                                             AppName, Name, Status);
                        Status = CFE_SUCCESS;
                    }
    ...
    if (Status < 0)
    {
        ...
        CFE_EVS_SendEventWithAppID(
            CFE_TBL_REGISTER_ERR_EID, CFE_EVS_ERROR, CFE_TBL_TaskData.TableTaskAppId,
            "%s Failed to Register '%s', Status=0x%08X", AppName, TblName, Status);
    }
...
int32 CFE_TBL_Share(CFE_TBL_Handle_t* TblHandlePtr, const char* TblName)
{
    ...
    if (Status < 0)
    {
        ...
        CFE_EVS_SendEventWithAppID(
            CFE_TBL_SHARE_ERR_EID, CFE_EVS_ERROR, CFE_TBL_TaskData.TableTaskAppId,
            "%s Failed to Share '%s', Status=0x%08X", AppName, TblName, Status);
    }
...
int32 CFE_TBL_Unregister(CFE_TBL_Handle_t TblHandle)
{
    ...
    if (Status < 0)
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_TBL_UNREGISTER_ERR_EID, CFE_EVS_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId,
                                   "%s Failed to Unregister '?', Status=0x%08X", AppName, Status);
    }
...
**
********************************************************************/
int32 CFE_TBL_LoadWorkingBuffer(CFE_TBL_Handle_t TblHandle, CFE_TBL_SrcEnum_t SrcType,
                                const void* SrcDataPtr)
{
    ...
    if (Status == CFE_SUCCESS)
    {
        ...
        {   /* Not DumpOnly */
            ...
            {
                /* In case the file contains a partial table load, get the active buffer contents
                 * first */
                memcpy(WorkingBufferPtr, RegRecPtr->Buffers[RegRecPtr->ActiveBufferIndex].BufferPtr,
                       RegRecPtr->Size);
            ...
            else if (SrcType == CFE_TBL_SRC_ADDRESS)
            {
                ...
                CFE_PSP_MemCpy((void*)WorkingBufferPtr, (void*)SrcDataPtr, RegRecPtr->Size);
            }
            else
            {
...
int32 UTF_TBL_LoadTableFromGround(char* Full_Table_Name, char* File_Name)
{
    ...
    /* Load working buffer */
    Status = CFE_TBL_LoadWorkingBuffer(TblHandle, CFE_TBL_SRC_FILE, (const void*)File_Name);
...
int32 CFE_TBL_Update(CFE_TBL_Handle_t TblHandle)
{
    ...
    if (Status == CFE_SUCCESS)
    {
        ...
        AccessDescPtr->Updated = TRUE;
        CFE_EVS_SendEventWithAppID(CFE_TBL_UPDATE_SUCCESS_INF_EID, CFE_EVS_INFORMATION,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s Successfully Updated '%s'",
    ...
    else
    {
        ...
        if (Status == CFE_SUCCESS)
        {
            CFE_EVS_SendEventWithAppID(CFE_TBL_UPDATE_SUCCESS_INF_EID, CFE_EVS_INFORMATION,
                                       CFE_TBL_TaskData.TableTaskAppId,
                                       "%s Successfully Updated '%s'", AppName, RegRecPtr->Name);
...
int32 CFE_TBL_ReleaseAddress(CFE_TBL_Handle_t TblHandle)
{
    ...
    {
        CFE_ES_WriteToSysLog(
            "CFE_TBL:ReleaseAddress-App(%d) does not have access to Tbl Handle=%d\n", ThisAppId,
            TblHandle);
    }
...
**                           If an address is not obtainable, the function returns a NULL for that
*ptr
...
int32 CFE_TBL_Validate(CFE_TBL_Handle_t TblHandle)
{
    ...
    if (Status == CFE_SUCCESS)
    {
        ...
        if (RegRecPtr->ValidateInactiveIndex != CFE_TBL_NO_VALIDATION_PENDING)
        {
            ...
            if (Status == CFE_SUCCESS)
            {
                CFE_EVS_SendEventWithAppID(CFE_TBL_VALIDATION_INF_EID, CFE_EVS_INFORMATION,
                                           CFE_TBL_TaskData.TableTaskAppId,
                                           "%s validation successful for Inactive '%s'", AppName,
                                           RegRecPtr->Name);
                RegRecPtr->LoadPending = TRUE;
            }
            else
            {
                CFE_EVS_SendEventWithAppID(CFE_TBL_VALIDATION_ERR_EID, CFE_EVS_ERROR,
                                           CFE_TBL_TaskData.TableTaskAppId,
            ...
            /* CFE_TBL_TaskData.ValidationResults[RegRecPtr->ValidateInactiveIndex].Result =
             * Status;*/

            /* Once validation is complete, set flags to indicate response is ready */
            /*CFE_TBL_TaskData.ValidationResults[RegRecPtr->ValidateInactiveIndex].State =
        ...
        else if (RegRecPtr->ValidateActiveIndex != CFE_TBL_NO_VALIDATION_PENDING)
        {
            ...
            if (Status == CFE_SUCCESS)
            {
                CFE_EVS_SendEventWithAppID(CFE_TBL_VALIDATION_INF_EID, CFE_EVS_INFORMATION,
                                           CFE_TBL_TaskData.TableTaskAppId,
                                           "%s validation successful for Active '%s'", AppName,
                                           RegRecPtr->Name);
            }
            else
            {
            ...
            CFE_TBL_TaskData.ValidationResults[RegRecPtr->ValidateActiveIndex].State =
                CFE_TBL_VALIDATION_PERFORMED;
            RegRecPtr->ValidateActiveIndex = CFE_TBL_NO_VALIDATION_PENDING;

            /* Since the validation was successfully performed (although maybe not a successful
             * result) */
...
int32 UTF_CFE_TBL_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_CFE_TB;_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_TBL_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        UTF_error("UTF Error: Read %d args w/script cmd UTF_SCRIPT_TBL_Set_Api_Return_Code. "
                  "Expected 2.\n",
                  argc - 1);
        UTF_exit();
    }
...
int32 CFE_TBL_Manage(CFE_TBL_Handle_t TblHandle)
{
    ...
    while (!FinishedManaging)
    {
        ...
        else if (Status == CFE_TBL_INFO_DUMP_PENDING)
        {
            ...
            /* After a Dump, always assume we are done (Dumps are on DumpOnly tables and cannot be
             * "Updated") */
            FinishedManaging = TRUE;
        }
