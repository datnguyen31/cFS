...
** The CFE_PSP_MemValidateRange function was modified to match the actual implementation in
*cFE 6.3.1.0
...
**    MemoryType -- The memory type to validate: CFE_PSP_MEM_RAM, CFE_PSP_MRM_EEPROM, or
*CFE_PSP_MEM_ANY
**
** Global Inputs: None
**
...
**   CFE_PSP_INVALID_MEM_TYPE -- Memory type associated with the range does not match the passed in
*type.
**   CFE_PSP_INVALID_MEM_RANGE -- The Memory range associated with the address is not large enough
*to contain
**                            Address + Size.
*/
int32 CFE_PSP_MemValidateRange(uint32 Address, uint32 Size, uint32 MemoryType)
{
    ...
    if (MemoryType != CFE_PSP_MEM_ANY && MemoryType != CFE_PSP_MEM_RAM &&
        MemoryType != CFE_PSP_MEM_EEPROM)
    {
    ...
    for (i = 0; i < CFE_PSP_MEM_TABLE_SIZE; i++)
    {
        ...
        */
        if (CFE_PSP_MemoryTable[i].MemoryType != CFE_PSP_MEM_INVALID)
        {
            ...
            */
            if ((StartAddressToTest >= StartAddressInTable) &&
                (StartAddressToTest <= EndAddressInTable))
            {
                /*
...
**    MemoryType -- The memory type to validate: CFE_PSP_MEM_RAM, CFE_PSP_MEM_EEPROM, or
*CFE_PSP_MEM_ANY
**    Address --    A 32 bit starting address of the memory range
**    Size    --    A 32 bit size of the memory range (Address + Size = End Address )
...
**   CFE_PSP_INVALID_MEM_TYPE -- Memory type associated with the range does not match the passed in
*type.
**   CFE_PSP_INVALID_MEM_SIZE -- The Memory range associated with the address is not large enough to
*contain
**                            Address + Size.
...
*/
int32 CFE_PSP_MemRangeSet(uint32 RangeNum, uint32 MemoryType, uint32 StartAddr, uint32 Size,
                          uint32 WordSize, uint32 Attributes)
{
    /* Handle Preset Return Code */
...
**    *Address --    A pointer to the 32 bit integer where the 32 bit starting address of the memory
*range
**                   is stored.
**    *EndAddr    --    A pointer to the 32 bit integer where the 32 bit size of the memory range
**                   is stored.
**    *WordSize --   A pointer to the 32 bit integer where the the minimum addressable size of the
*range:
**                     ( CFE_PSP_MEM_SIZE_BYTE, CFE_PSP_MEM_SIZE_WORD, CFE_PSP_MEM_SIZE_DWORD )
**    Attributes -- The attributes of the Memory Range:
**                     (CFE_PSP_MEM_ATTR_WRITE, CFE_PSP_MEM_ATTR_READ, CFE_PSP_MEM_ATTR_READWRITE)
...
**   CFE_PSP_INVALID_MEM_RANGE -- The index into the table is invalid
*/
int32 CFE_PSP_MemRangeGet(uint32 RangeNum, uint32* MemoryType, uint32* StartAddr, uint32* Size,
                          uint32* WordSize, uint32* Attributes)
{
    /* Handle Preset Return Code */
    ...
    if (MemoryType == NULL || StartAddr == NULL || Size == NULL || WordSize == NULL ||
        Attributes == NULL)
    {
