...
UTF_PSP_HookTable_t UTF_PSP_HookTable = {NULL, NULL, NULL, NULL, NULL};
...
{
    if (Index == CFE_PSP_EEPROMWRITE8_HOOK)
    {
        UTF_PSP_HookTable.CFE_PSP_EepromWrite8 = FunPtr;
    }
    else if (Index == CFE_PSP_EEPROMWRITE16_HOOK)
    {
        UTF_PSP_HookTable.CFE_PSP_EepromWrite16 = FunPtr;
    }
    else if (Index == CFE_PSP_EEPROMWRITE32_HOOK)
    {
        UTF_PSP_HookTable.CFE_PSP_EepromWrite32 = FunPtr;
    }
    else if (Index == CFE_PSP_EEPROMWRITEENA_HOOK)
    {
        UTF_PSP_HookTable.CFE_PSP_EepromWriteEnable = FunPtr;
    }
    else if (Index == CFE_PSP_EEPROMWRITEDIS_HOOK)
    {
        UTF_PSP_HookTable.CFE_PSP_EepromWriteDisable = FunPtr;
    }
    else
    {
        UTF_error("Invalid PSP Hook Index In Set Hook Call %d", Index);
    }
}
