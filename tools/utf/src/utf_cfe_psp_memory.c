...
int32 UTF_PSP_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_PSP_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_PSP_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        UTF_error("UTF Error: Read %d args w/script cmd UTF_SCRIPT_PSP_Set_Api_Return_Code. "
                  "Expected 2.\n",
                  argc - 1);
        UTF_exit();
    }
...
int32 CFE_PSP_GetKernelTextSegmentInfo(void* PtrToKernelSegment, uint32* SizeOfKernelSegment)
{
    ...
    if (cfe_psp_return_value[CFE_PSP_GETKERNELTEXTSEGMENTINFO_PROC] !=
        UTF_CFE_USE_DEFAULT_RETURN_CODE)
    {
    ...
    else
    {
        ...
        *SizeOfKernelSegment =
            (uint32)((uint32)&CCSDS_ComputeCheckSum - (uint32)&CFE_ES_RegisterApp);
        return_code = CFE_PSP_SUCCESS;
    }
...
int32 CFE_PSP_GetCFETextSegmentInfo(void* PtrToCFESegment, uint32* SizeOfCFESegment)
{
    ...
    else
    {
        ...
        *SizeOfCFESegment = (uint32)((uint32)&CCSDS_ComputeCheckSum - (uint32)&CFE_ES_RegisterApp);
        return_code       = CFE_PSP_SUCCESS;
    }
