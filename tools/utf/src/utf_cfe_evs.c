...
int32 EVS_SendEventInternal(uint32 AppID, CFE_TIME_SysTime_t Time, uint16 EventID, uint16 EventType,
                            char* EventString);
...
int32 CFE_EVS_SendEvent(uint16 EventID, uint16 EventType, const char* Spec, ...)
{
    ...
    if (Status == CFE_SUCCESS)
    {
        if (UTF_AppID_Registered == FALSE)
        {
            ...
            strcpy(warning_text,
                   "UTF WARNING: CFE_EVS_SendEvent called by application which is not ");
                                 ...
                                 ...)
{
    ...
    /* Handle Preset Return Code */
    if (cfe_evs_api_return_value[CFE_EVS_SENDEVENTWITHAPPID_PROC] !=
...
*/
int32 CFE_EVS_SendTimedEvent(CFE_TIME_SysTime_t Time, uint16 EventID, uint16 EventType,
                             const char* Spec, ...)
{
    char             BigBuf[CFE_EVS_MAX_MESSAGE_LENGTH];
    char             GTBigBuf[CFE_EVS_MAX_MESSAGE_LENGTH + 2];
...
int32 UTF_CFE_EVS_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_CFE_EVS_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_EVS_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        UTF_error("UTF Error: Read %d args w/script cmd UTF_SCRIPT_EVS_Set_Api_Return_Code. "
                  "Expected 2.\n",
                  argc - 1);
        UTF_exit();
    }
