...
** Added documentation and removed code in OS_mkfs to create the file system directory.  For unit
*testing this directory should already exist.
...
**        2. by making a call to UTF_add_volume to set only the device name and physical device and
*then call OS_mkfs and OS_mount
**           The only time you would want to use option 2 is where the task you are testing calls
*OS_mkfs and OS_mount.  Most of
...
**    - the virtual filesystem is "/cf", i.e. references to the virtual directory "/cf" get
*translated
...
void UTF_add_volume(char* DeviceName, char* PhysDevName, uint32 VolumeType, uint8 VolatileFlag,
                    uint8 FreeFlag, uint8 IsMounted, char* VolumeName, char* MountPoint,
                    uint32 BlockSize)
{
    int i;
