...
**  Added the capability for the specified hook functions. Added return code setting code for
*several
...
uint32 CFE_ES_MemPoolDefSize[CFE_ES_MAX_MEMPOOL_BLOCK_SIZES] = {
    CFE_ES_MAX_BLOCK_SIZE,    CFE_ES_MEM_BLOCK_SIZE_16, CFE_ES_MEM_BLOCK_SIZE_15,
    CFE_ES_MEM_BLOCK_SIZE_14, CFE_ES_MEM_BLOCK_SIZE_13, CFE_ES_MEM_BLOCK_SIZE_12,
    CFE_ES_MEM_BLOCK_SIZE_11, CFE_ES_MEM_BLOCK_SIZE_10, CFE_ES_MEM_BLOCK_SIZE_09,
    CFE_ES_MEM_BLOCK_SIZE_08, CFE_ES_MEM_BLOCK_SIZE_07, CFE_ES_MEM_BLOCK_SIZE_06,
    CFE_ES_MEM_BLOCK_SIZE_05, CFE_ES_MEM_BLOCK_SIZE_04, CFE_ES_MEM_BLOCK_SIZE_03,
    CFE_ES_MEM_BLOCK_SIZE_02, CFE_ES_MEM_BLOCK_SIZE_01};
...
UTF_ES_HookTable_t UTF_ES_HookTable = {NULL, NULL, NULL, NULL};
...
{
    if (Index == CFE_ES_POOLCREATE_HOOK)
    {
        UTF_ES_HookTable.CFE_ES_PoolCreate = FunPtr;
    }
    else if (Index == CFE_ES_POOLCREATENOSEM_HOOK)
    {
        UTF_ES_HookTable.CFE_ES_PoolCreateNoSem = FunPtr;
    }
    else if (Index == CFE_ES_GETPOOLBUF_HOOK)
    {
        UTF_ES_HookTable.CFE_ES_GetPoolBuf = FunPtr;
    }
    else if (Index == CFE_ES_PUTPOOLBUF_HOOK)
    {
        UTF_ES_HookTable.CFE_ES_PutPoolBuf = FunPtr;
    }
    else
    {
        UTF_error("Invalid ES Hook Index In Set Hook Call %d", Index);
    }
}
...
int32 CFE_ES_PoolCreateEx(CFE_ES_MemHandle_t* HandlePtr, uint8* MemPtr, uint32 Size,
    ...
    */
    ...
    { /* must be able make Pool management structure and at least one of the smallest blocks  */
        CFE_ES_WriteToSysLog(
            "CFE_ES:poolCreate Pool size(%d) too small for management structure, need >=(%d)\n",
            (int)Size, (int)(sizeof(Pool_t) + MinBlockSize + sizeof(BD_t)));
...
int32 CFE_ES_GetPoolBuf(uint32** BufPtr, CFE_ES_MemHandle_t HandlePtr, uint32 Size)
{
    ...
    */
    ...
    else /* go make one */
    {
        ...
        */
        PoolPtr->Current = (uint32*)((uint8*)PoolPtr->Current + Block + sizeof(BD_t));
    }
...
int32 CFE_ES_PutPoolBuf(CFE_ES_MemHandle_t Handle, uint32* BufPtr)
{
    ...
    if (((uint32)BdPtr < Handle) || ((uint32)BdPtr >= (PoolPtr->End - sizeof(BD_t))))
    {
        /* sanity check */
        CFE_ES_WriteToSysLog(
            "CFE_ES:putPool-Invalid Memory Handle (0x%08X) or memory block (0x%08X).\n",
...
uint32 CFE_ES_GetBlockSize(Pool_t* PoolPtr, uint32 Size)
{
    ...
    while ((i < (CFE_ES_MAX_MEMPOOL_BLOCK_SIZES - 1)) && (Size <= PoolPtr->SizeDesc[i + 1].MaxSize))
    {
        i++;
    }
...
boolean CFE_ES_ValidateHandle(CFE_ES_MemHandle_t Handle)
{
    ...
    else
    {
        ...
        /* Step #3) The first field of the pool structure should contain the start address of the
         * pool */
        if (Handle != (CFE_ES_MemHandle_t)PoolPtr->Start)
        {
            HandleValid = FALSE;
        ...
        /* Step #5) The pool structure should have an End pointer that is equal to the start plus
         * the size */
