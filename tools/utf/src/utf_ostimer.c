...
int32 UTF_OSTIMER_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_OSTIMER_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_OSTIMER_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        UTF_error("UTF Error: Read %d args w/script cmd UTF_SCRIPT_OSTIMER_Set_Api_Return_Code. "
                  "Expected 2.\n",
                  argc - 1);
        UTF_exit();
    }
