...
#define CDS_POOL_OFFSET                                                                            \
    (((CDS_REG_OFFSET + (CFE_ES_CDS_MAX_NUM_ENTRIES * sizeof(CFE_ES_CDS_RegRec_t))) + 3) &         \
     0xfffffffc)
...
int32 CFE_ES_CDS_EarlyInit(void)
{
    ...
    MinRequiredSize =
        (sizeof(CDSVars.ValidityField) * 2) + /* Minimum size for validity fields */
        (CFE_ES_CDS_MAX_NUM_ENTRIES *
         sizeof(CFE_ES_CDS_RegRec_t)) + /* Minimum size for CDS Registry contents */
    ...
    /* If the size was obtained successfully and meets the minimum requirements, then check its
     * contents */
...
********************************************************************/
int32 CFE_ES_RegisterCDSEx(CFE_ES_CDSHandle_t* HandlePtr, int32 BlockSize, const char* Name,
                           boolean CriticalTbl)
{
    int32                Status    = CFE_SUCCESS;
...
int32 CFE_ES_InitializeCDS(uint32 CDSSize)
{
    ...
    else
    {
        ...
        if (Status == OS_SUCCESS)
        {
            Status = CFE_PSP_WriteToCDS("_CDSEnd_", (CDSSize - sizeof(CDSVars.ValidityField)),
                                        sizeof(CDSVars.ValidityField));
            if (Status != OS_SUCCESS)
            {
            ...
            else
            {
                ...
                /* Total CDS Size minus the validity fields and the space for the registry and the
                 * registry size */
...
int32 CFE_ES_InitCDSRegistry(void)
{
    ...
    Status = CFE_PSP_WriteToCDS(&CDSVars.MaxNumRegEntries, CDS_REG_SIZE_OFFSET,
...
int32 CFE_ES_UpdateCDSRegistry(void)
{
    ...
    Status = CFE_PSP_WriteToCDS(CDSVars.Registry, CDS_REG_OFFSET, sizeof(CDSVars.Registry));
...
int32 CFE_ES_RebuildCDS(void)
{
    ...
    /* Copy the number of registry entries to the CDS */
    Status = CFE_PSP_ReadFromCDS(&CDSVars.MaxNumRegEntries, CDS_REG_SIZE_OFFSET,
                                 sizeof(CDSVars.MaxNumRegEntries));

    if (CDSVars.MaxNumRegEntries <= CFE_ES_CDS_MAX_NUM_ENTRIES)
    {
        Status = CFE_PSP_ReadFromCDS(&CDSVars.Registry, CDS_REG_OFFSET,
        ...
        if (Status == OS_SUCCESS)
        {
            ...
            PoolOffset =
                (CDS_REG_OFFSET + (CDSVars.MaxNumRegEntries * sizeof(CFE_ES_CDS_RegRec_t)) + 3) &
                0xfffffffc;
            ;
    ...
    if (RegIndx != CFE_ES_CDS_NOT_FOUND)
    {
        ...
        else
        {
            ...
            if (Status == CFE_ES_ERR_APPNAME)
            {
                ...
                if (Status < 0)
                {
                    CFE_ES_WriteToSysLog("CFE_ES:DeleteCDS-Failed to free CDS Mem Block "
                                         "(Handle=0x%08X)(Stat=0x%08X)\n",
