...
** Removed feature to automatically print any sim mem access, as soon as I made it automatic people
*were asking me how to disable it.  So now you have to do it the old way using hooks.
...
** Local prototypes
*/
static void read_hex_data(utf_file_context_type* fc, void* buffer, unsigned short int max_bytes,
...
void UTF_add_sim_address(unsigned long start, unsigned long size, char* name)
{
    if (sim_address_count < UTF_SIM_MEM_LOOKUP_TABLE_SIZE)
    {
        ...
        if (size > 0)
        {
            ...
            {
                UTF_error("Sim Mem Error Allocating Sim Memory Space, Address 0X%x, Bytes %d",
                          start, size);
            }
    ...
    {
        UTF_error("Sim Mem Maximum Number of Memory Addresses Exceeded, Max: %d",
                  UTF_SIM_MEM_LOOKUP_TABLE_SIZE);
    }
}
...
void* UTF_sim_address_lookup(unsigned long int address, unsigned short int size)
{
    ...
    { /* Check if starting address is within range */
            ...
            (address < sim_memory_lookup_table[i].start + sim_memory_lookup_table[i].size))
        {
            ...
            if ((address - sim_memory_lookup_table[i].start) + size >
                sim_memory_lookup_table[i].size)
            {
                UTF_put_text(
                    "UTF ERROR: Requested simulated memory access extends beyond segment end.\n");
                ...
                return ((void*)(sim_memory_lookup_table[i].memory +
                                (address - sim_memory_lookup_table[i].start)));
        }
    }
...
char UTF_read_sim_address(unsigned long int address, unsigned short int num_bytes, void* data_ptr)
{
    ...
    if (read_sim_hook == NULL || read_sim_hook(address, num_bytes, data_ptr) == FALSE)
    {
...
char UTF_write_sim_address(unsigned long int address, unsigned short int num_bytes, void* data_ptr)
{
    ...
    if (write_sim_hook == NULL || write_sim_hook(address, num_bytes, data_ptr) == FALSE)
    {
        sim_memory_ptr = UTF_sim_address_lookup(address, num_bytes);
        if (sim_memory_ptr != NULL)
        {
...
void UTF_add_input_queue(unsigned long PipeId)
{
    ...
    else if (queue_table_count > UTF_QUEUE_COUNT)
    { /* 1 based array */
    ...
    else
    {
        ...
        file_descriptor_table[file_descriptor_count].queue_id =
            queue_table_count + 1; /* index into queue_table, must be > 0 */
        file_descriptor_table[file_descriptor_count].file_context.filename =
            NULL; /* not used for queues */
        file_descriptor_table[file_descriptor_count].file_context.fd =
            NULL; /* not used for queues */
        file_descriptor_table[file_descriptor_count].file_context.line_number =
            0; /* not used for queues */
...
void UTF_add_special_command(char* name, utf_special_command_type function)
{
    ...
    if (function != NULL)
    {
        ...
        {
            UTF_error("Maximum Number of Special Commands Exceeded, Max: %d",
                      UTF_SPECIAL_COMMAND_TABLE_SIZE);
        }
    }
    else
    {
...
void UTF_process_special_command(utf_file_context_type* fc, char* command_line)
{
    ...
    if (argc > 0)
    {
        ...
        {
            UTF_error("File: %s, Line: %d, Special Command Not Found, %s", fc->filename,
                      fc->line_number, argv[0]);
        }
    }
    else
    {
        UTF_error("File: %s, Line: %d, Special Command Name Is Missing", fc->filename,
                  fc->line_number);
    }
}
    ...
    *argc = 0;
    ...
    while (*argc < UTF_SPECIAL_COMMAND_MAX_ARGS)
    {
        ...
        while ((c = *ptr) != '\0' && isspace(c))
            ptr++; /* skip blank spaces */
        if (c == '\0')
            break;             /* at the end of the string so quit */
        argv[(*argc)++] = ptr; /* save a ptr to the start of the argument */
        while ((c = *ptr) != '\0' && !isspace(c))
            ptr++;   /* find the next space */
        *ptr = '\0'; /* replace the space with a null terminator */
        if (c != '\0')
            ptr++; /* skip to the next character */
    }
}
...
utf_file_context_type* UTF_get_input_fc(unsigned long source_id)
{
    ...
    for (i = 0; i < file_descriptor_count; i++)
    {
        if (source_id == file_descriptor_table[i].source_id)
        {
            ...
            {
                file_descriptor_table[i].file_context.fd =
                    fopen(file_descriptor_table[i].file_context.filename, "r");
                if (file_descriptor_table[i].file_context.fd == NULL)
                {

                    UTF_error("Error Opening Input File %s",
                              file_descriptor_table[i].file_context.filename);
                }
            }
...
*/
char UTF_get_data(unsigned long source_id, void* buffer, unsigned short int length,
                  unsigned short int data_type)
{
    ...
    switch (data_type)
    {
        ...
        case UTF_AS_BYTE:
                ...
                *byte_ptr = 0;
                ...
                for (i = 0; i < 2; i++)
                {
                    if (token == END_OF_LINE)
                        UTF_error("File: %s, Line: %d, Not Enough Characters For AS_BYTE Read",
                                  fc->filename, fc->line_number - 1);
                    if (token != NUMBER)
                        UTF_error("File: %s, Line: %d, Invalid Hex Number Read", fc->filename,
                                  fc->line_number);
            ...
            if ((token != END_OF_LINE) && (bytes_read >= max_bytes))
                UTF_error("File: %s, Line: %d, Input Data Exceeds Max Size of %d", fc->filename,
                          fc->line_number, max_bytes);
        ...
        case UTF_AS_WORD:
            ...
            if ((max_bytes % 2) != 0)
                UTF_error("File: %s, Line: %d, Invalid Max_Bytes Parameter For AS_WORD Read, %d",
                          fc->filename, fc->line_number, max_bytes);
            ...
            while ((token != END_OF_LINE) && (bytes_read < max_bytes))
            {
                ...
                *word_ptr = 0;
                ...
                {
                    if (token == END_OF_LINE)
                        UTF_error("File: %s, Line: %d, Not Enough Characters For AS_WORD Read",
                                  fc->filename, fc->line_number - 1);
                    if (token != NUMBER)
                        UTF_error("File: %s, Line: %d, Invalid Hex Number Read", fc->filename,
                                  fc->line_number);
        ...
        case UTF_AS_DWORD:
            ...
            if ((max_bytes % 4) != 0)
                UTF_error("File: %s, Line: %d, Invalid Max_Bytes Parameter For AS_DWORD Read, %d",
                          fc->filename, fc->line_number, max_bytes);
            ...
            while ((token != END_OF_LINE) && (bytes_read < max_bytes))
            {
                ...
                *dword_ptr = 0;
                ...
                {
                    if (token == END_OF_LINE)
                        UTF_error("File: %s, Line: %d, Not Enough Characters For AS_DWORD Read",
                                  fc->filename, fc->line_number - 1);
                    if (token != NUMBER)
                        UTF_error("File: %s, Line: %d, Invalid Hex Number Read", fc->filename,
                                  fc->line_number);
        ...
        default:
            UTF_error("File: %s, Line: %d, Invalid DATA_TYPE For Read, %d", fc->filename,
                      fc->line_number, data_type);
            break;
    }
}
    ...
    while (TRUE)
    {
        ...
        switch (ch)
        {
            ...
            case '*': /* label */
                ...
                UTF_error("File: %s, Line: %d, Label Exceeds %d Characters, %s", fc->filename,
                          fc->line_number, UTF_STRING_TOKEN_SIZE, string_token);
                break;
            ...
            case '@': /* special command */
                ...
                UTF_error("File: %s, Line: %d, Special Command Exceeds %d Characters, %s",
                          fc->filename, fc->line_number, UTF_STRING_TOKEN_SIZE, string_token);
                break;
