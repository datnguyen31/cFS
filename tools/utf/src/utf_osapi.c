...
** The OS_API_Init() function calls a new FS api (OS_FS_Init()) function. This call was added to the
*UTF
** implementation.
...
** Added the OS_TaskDelete function hook capability to utf_osapi and the example test 14. I also
*changed
...
** 06/23/05 Copied from osapi.c in cFE Build 1. Functions have been gutted to provide baseline for
*cFE UTF.  N. Schweiss
*/
...
UTF_OSAPI_HookTable_t UTF_OSAPI_HookTable = {NULL, NULL, NULL, NULL, NULL,
...
{
    if (Index == OS_TASKDELAY_HOOK)
    {
        UTF_OSAPI_HookTable.OS_TaskDelay = FunPtr;
    }
    else if (Index == OS_BINSEMTAKE_HOOK)
    {
        UTF_OSAPI_HookTable.OS_BinSemTake = FunPtr;
    }
    else if (Index == OS_BINSEMTIMEDWAIT_HOOK)
    {
        UTF_OSAPI_HookTable.OS_BinSemTimedWait = FunPtr;
    }
    else if (Index == OS_MUTSEMTAKE_HOOK)
    {
        UTF_OSAPI_HookTable.OS_MutSemTake = FunPtr;
    }
    else if (Index == OS_GETLOCALTIME_HOOK)
    {
        UTF_OSAPI_HookTable.OS_GetLocalTime = FunPtr;
    }
    else if (Index == OS_QUEUEGET_HOOK)
    {
        UTF_OSAPI_HookTable.OS_QueueGet = FunPtr;
    }
    else if (Index == OS_QUEUEPUT_HOOK)
    {
        UTF_OSAPI_HookTable.OS_QueuePut = FunPtr;
    }
    else if (Index == OS_TASKDELETE_HOOK)
    {
        UTF_OSAPI_HookTable.OS_TaskDelete = FunPtr;
    }
    else if (Index == OS_BINSEMGIVE_HOOK)
    {
        UTF_OSAPI_HookTable.OS_BinSemGive = FunPtr;
    }
    else if (Index == OS_BINSEMCREATE_HOOK)
    {
        UTF_OSAPI_HookTable.OS_BinSemCreate = FunPtr;
    }
    else
    {
        UTF_error("Invalid OSAPI Hook Index In Set Hook Call %d", Index);
    }
}
...
{
    UTF_put_text("UTF Version: %d.%d.%d.%c\n", UTF_MAJOR_VERSION, UTF_MINOR_VERSION,
                 UTF_SUBMINOR_VERSION, UTF_REVISION);
}
...
int32 UTF_CFE_OSAPI_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_CFE_SB_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_OSAPI_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    {
        UTF_error(
            "UTF Error: Read %d args w/script cmd UTF_SCRIPT_SB_Set_Api_Return_Code. Expected 2.\n",
            argc - 1);
        UTF_exit();
    }
...
void UTF_SCRIPT_OSAPI_Use_Default_Api_Return_Code(int argc, char* argv[])
{
    ...
    {
        UTF_error(
            "UTF Error: Read %d args w/script cmd UTF_SCRIPT_SB_Set_Api_Return_Code. Expected 1.\n",
            argc - 1);
        UTF_exit();
    }
...
---------------------------------------------------------------------------------------*/
int32 OS_QueueCreate(uint32* queue_id, const char* queue_name, uint32 queue_depth, uint32 data_size,
                     uint32 flags)
{
...
/*---------------------------------------------------------------------------------------
    ...
    Purpose: The function  unlocks the semaphore referenced by sem_id by performing
             a semaphore unlock operation on that semaphore.If the semaphore value
             resulting from this operation is positive, then no threads were blocked waiting for the
semaphore to become unlocked; the semaphore value is simply incremented for this semaphore.
...
int32 OS_ExcAttachHandler(uint32 ExceptionNumber, void (*ExceptionHandler)(uint32, uint32*, uint32),
                          int32  Parameter)
{
