...
** version of CFE_SB_RcvMsg. If this return code is set and an enpty pipe is encountered, the test
*process will exit.
...
** Added the implementation of the CFE_DeletePipe function. Also, the capability to define a hook
*function
...
UTF_SB_HookTable_t UTF_SB_HookTable = {NULL, NULL, NULL, NULL};
...
{
    if (Index == CFE_SB_SUBSCRIBE_HOOK)
    {
        UTF_SB_HookTable.CFE_SB_Subscribe = FunPtr;
    }
    else if (Index == CFE_SB_SUBSCRIBEEX_HOOK)
    {
        UTF_SB_HookTable.CFE_SB_SubscribeEx = FunPtr;
    }
    else if (Index == CFE_SB_CREATEPIPE_HOOK)
    {
        UTF_SB_HookTable.CFE_SB_CreatePipe = FunPtr;
    }
    else if (Index == CFE_SB_DELETEPIPE_HOOK)
    {
        UTF_SB_HookTable.CFE_SB_DeletePipe = FunPtr;
    }
    else
    {
        UTF_error("Invalid SB Hook Index In Set Hook Call %d", Index);
    }
}
...
**
******************************************************************************/
int32 CFE_SB_SubscribeEx(CFE_SB_MsgId_t MsgId, CFE_SB_PipeId_t PipeId, CFE_SB_Qos_t Quality,
                         uint16 MsgLim)
{
    ...
    else /* no data available */
    {
        ...
        switch (TimeOut)
        {
            case CFE_SB_PEND_FOREVER:
                ...
                if (cfe_sb_api_return_value[CFE_SB_RCVMSG_PROC] ==
                    UTF_CFE_SB_RCVMSG_EXIT_RETURN_CODE)
                {
                    UTF_exit();
                }
