...
** The CFE_ES_GetAppInfo function along with an internal function to set this info and the test that
*calls
...
** Modifications to mimic the cFE 4.1.0 ES fsw. These changes include new APIs as well as Task info
*storage.
...
** Most of the changes to these files was to add the copyright comments. There were other changes
*needed
...
** Provided additional utility to allow default api behavior to be restored with procedure call from
*test driver.
...
UTF_ES_API_HookTable_t UTF_ES_API_HookTable = {NULL};
    ...
    if (ResetType == CFE_ES_PROCESSOR_RESET)
    {
        if (utf_ProcessorResetCount == CFE_ES_MAX_PROCESSOR_RESETS)
        {
            CFE_ES_WriteToSysLog(
                "ES ResetCFE: CFE ES Power On Reset Due to Max Processor Resets.\n");
...
int32 CFE_ES_RestartApp(uint32 AppID)
{
    ...
    */
    ...
    {
        CFE_ES_WriteToSysLog(
            "CFE_ES_RestartApp: Cannot Restart Application %s, It is not running.\n",
            ES_AppTable[AppID].StartParams.Name);
        ReturnCode = CFE_ES_ERR_APPID;
    }
    else
...
int32 CFE_ES_ReloadApp(uint32 AppID, const char* AppFileName)
{
    ...
    */
    ...
    {
        CFE_ES_WriteToSysLog(
            "CFE_ES_ReloadApp: Reload Application %s Initiated. New filename = %s\n",
    ...
    */
    ...
    else
    {
        ...
        if (ES_AppTable[AppID].TaskInfo.NumOfChildTasks > 0)
        {
            ...
            {
                if ((ES_TaskTable[i].RecordUsed == TRUE) && (ES_TaskTable[i].AppId == AppID))
                {
...
int32 CFE_ES_RunLoop(uint32* RunStatus)
{
    ...
    ** Validate RunStatus
    */
    if (*RunStatus == CFE_ES_APP_RUN || *RunStatus == CFE_ES_APP_EXIT ||
        *RunStatus == CFE_ES_APP_ERROR)
    {
        ...
        */
        ...
        {
            CFE_ES_WriteToSysLog(
                "CFE_ES_RunLoop Error: Cannot get AppID for the caller: RC = %08X\n", ReturnCode);
            ReturnCode = FALSE;
...
int32 UTF_ES_AddAppRecord(char* AppName, uint32 AppId)
{
    ...
    {
        UTF_put_text("UTF Info: App name <%s> is too long for App Tbl. (Max %d chars incl. null "
                     "terminator.)\n",
...
int32 UTF_ES_AddTaskRecord(char* TaskName, uint32 AppId, uint32 TaskId)
{
    ...
    {
        UTF_put_text("UTF Info: Task name <%s> is too long for App Tbl. (Max %d chars incl. null "
                     "terminator.)\n",
...
void CFE_ES_GetAppInfoInternal(uint32 AppId, CFE_ES_AppInfo_t* AppInfoPtr)
{
    ...
    strncpy((char*)AppInfoPtr->EntryPoint, (char*)ES_AppTable[AppId].StartParams.EntryPoint,
            OS_MAX_API_NAME);
    AppInfoPtr->EntryPoint[OS_MAX_API_NAME - 1] = '\0';

    strncpy((char*)AppInfoPtr->FileName, (char*)ES_AppTable[AppId].StartParams.FileName,
            OS_MAX_PATH_LEN);
    ...
    strncpy((char*)AppInfoPtr->MainTaskName, (char*)ES_AppTable[AppId].TaskInfo.MainTaskName,
            OS_MAX_API_NAME);
    ...
    if (TaskId < OS_MAX_TASKS)
    {
        if (ES_TaskTable[TaskId].RecordUsed == TRUE)
        {
            ...
            */
            ...
            strncpy((char*)TaskInfo->TaskName, (char*)ES_TaskTable[TaskId].TaskName,
                    OS_MAX_API_NAME);
            ...
            */
            if (ES_AppTable[TaskInfo->AppId].RecordUsed == TRUE)
            {
                strncpy((char*)TaskInfo->AppName,
                        (char*)ES_AppTable[TaskInfo->AppId].StartParams.Name, OS_MAX_API_NAME);
            ...
            {
                CFE_ES_WriteToSysLog(
                    "CFE_ES_GetTaskInfo: Task ID:%d Parent App ID:%d not Active.\n", TaskId,
                    TaskInfo->AppId);
...
int32 CFE_ES_CreateChildTask(uint32* TaskIdPtr, const char* TaskName,
                             CFE_ES_ChildTaskMainFuncPtr_t FunctionPtr, const uint32* StackPtr,
                             uint32 StackSize, uint32 Priority, uint32 Flags)
{
    ...
    if (UTF_ES_API_HookTable.CFE_ES_CreateChildTask)
        return (UTF_ES_API_HookTable.CFE_ES_CreateChildTask(TaskIdPtr, TaskName, FunctionPtr,
                                                            StackPtr, StackSize, Priority, Flags));
    ...
    */
    ...
    else
    {
        ...
        */
        ...
        {
            CFE_ES_WriteToSysLog(
                "CFE_ES_CreateChildTask: Error calling CFE_ES_GetAppID. RC = 0x%08X\n", Result);
            ReturnCode = Result;
        }
            ...
            */
            ...
            if (Result != -1)
            {
                ...
                */
                ...
                {
                    CFE_ES_WriteToSysLog(
                        "CFE_ES_CreateChildTask: Error calling OS_TaskCreate RC = 0x%08X\n",
                        Result);
            ...
            */
            ...
            if (TaskIsMain == FALSE)
            {
                ...
                */
                ...
                {
                    CFE_ES_WriteToSysLog("CFE_ES_DeleteChildTask Error: Error Calling "
                                         "OS_TaskDelete: Task %d, RC = 0x%08X\n",
                                         TaskId, OSReturnCode);
...
void CFE_ES_ExitChildTask(void)
{
    ...
    */
    ...
    if (ReturnCode == CFE_SUCCESS)
    {
        ...
        {
            CFE_ES_WriteToSysLog(
                "CFE_ES_ExitChildTask Error: Cannot Call from a cFE App Main Task. ID = %d\n",
                TaskId);
        }
    }
    else
    {
        CFE_ES_WriteToSysLog(
            "CFE_ES_ExitChildTask Error Calling CFE_ES_GetAppID. Task ID = %d, RC = 0x%08X\n",
            TaskId, ReturnCode);
    } /* end if GetAppId */
...
int32 CFE_ES_RegisterCDS(CFE_ES_CDSHandle_t* CDSHandlePtr, int32 BlockSize, const char* Name)
{
    ...
    if (Status < 0)
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_ES_CDS_REGISTER_ERR_EID, CFE_EVS_ERROR, ThisAppId,
...
int32 UTF_CFE_ES_Use_Default_Api_Return_Code(int32 ApiIndex)
{
    ...
    {
        UTF_put_text(
            "UTF Error: Invalid ApiIndex %d passed to UTF_CFE_ES_Use_Default_Api_Return_Code\n",
            ApiIndex);
        return -1;
    }
...
void UTF_SCRIPT_ES_Set_Api_Return_Code(int argc, char* argv[])
{
    ...
    {
        UTF_error(
            "UTF Error: Read %d args w/script cmd UTF_SCRIPT_ES_Set_Api_Return_Code. Expected 2.\n",
            argc - 1);
        UTF_exit();
    }
...
void UTF_SCRIPT_ES_Use_Default_Api_Return_Code(int argc, char* argv[])
{
    ...
    {
        UTF_error(
            "UTF Error: Read %d args w/script cmd UTF_SCRIPT_ES_Set_Api_Return_Code. Expected 1.\n",
            argc - 1);
        UTF_exit();
    }
