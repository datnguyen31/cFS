...
int32 CFE_TBL_EarlyInit(void)
{
    ...
    */
    CFE_SB_InitMsg(&CFE_TBL_TaskData.HkPacket, CFE_TBL_HK_TLM_MID, sizeof(CFE_TBL_HkPacket_t),
                   TRUE);
    ...
    */
    ...
    Status = CFE_ES_PoolCreate(&CFE_TBL_TaskData.Buf.PoolHdl, CFE_TBL_TaskData.Buf.Partition,
    ...
    else
    {
        ...
        while ((j < CFE_TBL_MAX_SIMULTANEOUS_LOADS) && (Status >= 0))
        {
            ...
            Status = CFE_ES_GetPoolBuf((uint32**)&CFE_TBL_TaskData.LoadBuffs[j].BufferPtr,
                                       CFE_TBL_TaskData.Buf.PoolHdl, CFE_TBL_MAX_SNGL_TABLE_SIZE);
    ...
    /* Assume for the moment that nothing is already in the CDS and zero out the Critical Table
     * Registry */
    ...
    if (Status == CFE_ES_CDS_ALREADY_EXISTS)
    {
        ...
        if (Status != CFE_SUCCESS)
        {
            ...
            CFE_ES_WriteToSysLog(
                "CFE_TBL:EarlyInit-Failed to recover Critical Table Registry (Err=0x%08X)\n",
                Status);
        }

        /* Whether we recovered the Critical Table Registry or not, we are successful with
         * initialization */
        Status = CFE_SUCCESS;
    }
    else if (Status != CFE_SUCCESS)
    {
        ...
        CFE_ES_WriteToSysLog(
            "CFE_TBL:EarlyInit-Failed to create Critical Table Registry (Err=0x%08X)\n", Status);

        /* Failure to support critical tables is not a good enough reason to exit the cFE on start
         * up */
        Status = CFE_SUCCESS;
    }
...
int32 CFE_TBL_ValidateHandle(CFE_TBL_Handle_t TblHandle)
{
    ...
    else
    {
        ...
        if (CFE_TBL_TaskData.Handles[TblHandle].UsedFlag == FALSE)
        {
            ...
            CFE_ES_WriteToSysLog(
                "CFE_TBL:ValidateHandle-Table Handle=%d is for unused Table Handle\n", TblHandle);
        }
    }
    return Status;
...
int32 CFE_TBL_RemoveAccessLink(CFE_TBL_Handle_t TblHandle)
{
    ...
    if (RegRecPtr->HeadOfAccessList == CFE_TBL_END_OF_LIST)
    {
        ...
        if (RegRecPtr->UserDefAddr == FALSE)
        {
            ...
            Status                          = CFE_ES_PutPoolBuf(CFE_TBL_TaskData.Buf.PoolHdl,
                                                                (uint32*)RegRecPtr->Buffers[0].BufferPtr);
            RegRecPtr->Buffers[0].BufferPtr = NULL;

            if (Status < 0)
            {
                CFE_ES_WriteToSysLog("CFE_TBL:RemoveAccessLink-PutPoolBuf[0] Fail Stat=0x%08X, "
                                     "Hndl=0x%08X, Buf=0x%08X\n",
                                     Status, CFE_TBL_TaskData.Buf.PoolHdl,
                                     RegRecPtr->Buffers[0].BufferPtr);
            }
            ...
            {
                Status                          = CFE_ES_PutPoolBuf(CFE_TBL_TaskData.Buf.PoolHdl,
                                                                    (uint32*)RegRecPtr->Buffers[1].BufferPtr);
                RegRecPtr->Buffers[1].BufferPtr = NULL;

                if (Status < 0)
                {
                    CFE_ES_WriteToSysLog("CFE_TBL:RemoveAccessLink-PutPoolBuf[1] Fail Stat=0x%08X, "
                                         "Hndl=0x%08X, Buf=0x%08X\n",
                                         Status, CFE_TBL_TaskData.Buf.PoolHdl,
                                         RegRecPtr->Buffers[1].BufferPtr);
                }
            }
            else
            {
...
int32 CFE_TBL_GetAddressInternal(void** TblPtr, CFE_TBL_Handle_t TblHandle, uint32 ThisAppId)
{
    ...
    if (Status == CFE_SUCCESS)
    {
        ...
        {
            CFE_ES_WriteToSysLog(
                "CFE_TBL:GetAddressInternal-App(%d) does not have access to Tbl Handle=%d\n",
                ThisAppId, TblHandle);
        }
    }
    else
...
int32 UTF_CFE_TBL_DisplayTableRegistryContents(void)
{
    ...
    do
    {
        ...
        /* if (CFE_TBL_TaskData.Registry[i].OwnerAppId != CFE_TBL_NOT_OWNED) */
        {
            ...
            UTF_put_text("%4d %24s %8d", i, RegRecPtr->Name, RegRecPtr->Size);
...
**
********************************************************************/
int32 CFE_TBL_LoadFromFile(CFE_TBL_LoadBuff_t* WorkingBufferPtr, CFE_TBL_RegistryRec_t* RegRecPtr,
                           const char* Filename)
{
    int32              Status = CFE_SUCCESS;
...
int32 CFE_TBL_ReadHeaders(int32 FileDescriptor, CFE_FS_Header_t* StdFileHeaderPtr,
                          CFE_TBL_File_Hdr_t* TblFileHeaderPtr, const char* LoadFilename)
{
    ...
    if (Status != sizeof(CFE_FS_Header_t))
    {
        ...
        CFE_EVS_SendEventWithAppID(
            CFE_TBL_FILE_STD_HDR_ERR_EID, CFE_EVS_ERROR, CFE_TBL_TaskData.TableTaskAppId,
            "Unable to read std header for '%s', Status = 0x%08X", LoadFilename, Status);
    ...
    else
    {
        ...
        {
            CFE_EVS_SendEventWithAppID(CFE_TBL_FILE_TYPE_ERR_EID, CFE_EVS_ERROR,
        ...
        else
        {
            ...
            {
                CFE_EVS_SendEventWithAppID(CFE_TBL_FILE_SUBTYPE_ERR_EID, CFE_EVS_ERROR,
                                           CFE_TBL_TaskData.TableTaskAppId,
            ...
            else
            {
                ...
                {
                    CFE_EVS_SendEventWithAppID(
                        CFE_TBL_FILE_TBL_HDR_ERR_EID, CFE_EVS_ERROR,
                        CFE_TBL_TaskData.TableTaskAppId,
                        "Unable to read tbl header for '%s', Status = 0x%08X", LoadFilename,
                        Status);
                ...
                else
                {
                    ...
                    if ((*(char*)&EndianCheck) == 0x04)
                    {
                         ...
                         */
                        /* which is in big endian format, must be swapped so that the data is
                         * readable */
...
int32 CFE_TBL_DumpToBuffer(CFE_TBL_Handle_t TblHandle)
{
    ...
    CFE_TBL_DumpControl_t*      DumpCtrlPtr =
        &CFE_TBL_TaskData.DumpControlBlocks[RegRecPtr->DumpControlIndex];
    CFE_TIME_SysTime_t DumpTime;

    /* Copy the contents of the active buffer to the assigned dump buffer */
    CFE_PSP_MemCpy(DumpCtrlPtr->DumpBufferPtr->BufferPtr, RegRecPtr->Buffers[0].BufferPtr,
                   DumpCtrlPtr->Size);
...
void CFE_TBL_FindCriticalTblInfo(CFE_TBL_CritRegRec_t** CritRegRecPtr,
...
void CFE_TBL_UpdateCriticalTblCDS(CFE_TBL_RegistryRec_t* RegRecPtr)
{
    ...
    Status = CFE_ES_CopyToCDS(RegRecPtr->CDSHandle,
                              RegRecPtr->Buffers[RegRecPtr->ActiveBufferIndex].BufferPtr);

    if (Status != CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog(
            "CFE_TBL:UpdateCritTbl-Unable to update Critical Table '%s' in CDS (Err=0x%08X)\n",
    ...
    else
    {
        ...
        {
            /* Save information related to the source of the data stored in the table in Critical
             * Table Registry */
            ...
            if (Status != CFE_SUCCESS)
            {
                CFE_ES_WriteToSysLog("CFE_TBL:UpdateCritTbl-Unable to update Critical Table "
                                     "Registry in CDS (Err=0x%08X)\n",
                                     Status);
            }
        }
        else
        {
            CFE_ES_WriteToSysLog(
                "CFE_TBL:UpdateCritTbl-Error finding '%s' in Critical Table Registry\n",
                RegRecPtr->Name);
        }
    }
