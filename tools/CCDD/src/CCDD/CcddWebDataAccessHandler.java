...
public class CcddWebDataAccessHandler extends AbstractHandler
{
    ...
    private String getQueryResults(String component, String item)
    {
        ...
        eventLog.logEvent(EventLogMessageType.SERVER_MSG,
                          new StringBuilder("Request component '").append(component)
                                                                  .append("' item '")
                                                                  .append((component.contains("authentic") ? item.replaceFirst("=.*(?:;|$)", "=*****")
                                                                                                           : item))
                                                                  .append("'"));
    ...
    private String getGroupNames(boolean applicationOnly)
    {
        ...
        if (groupNames.length != 0)
        {
            ...
            responseJO.put((applicationOnly ? JSONTags.APPLICATION_NAMES.getTag()
        ...
        else if (appTableType.equals("message"))
        {
            ...
            if (msgEntries.length != 0)
            {
                ...
                for (int row = 0; row < msgEntries.length; row++)
                {
                    ...
                    rowJO.put("Command ID #" + row,
                              msgEntries[row] + (!msgEntries[row].equals("SCH_UNUSED_MID") ? ", 0xC000, 0x0001, 0x0000"
                                                                                           : ""));
    ...
    private String getVariableNames(String variablePath, String parameters) throws CCDDException
    {
        ...
        if (separators != null)
        {
            ...
            if (!variablePath.isEmpty())
            {
                ...
                responseJO.put(variablePath, variableHandler.getFullVariableName(variablePath,
                                                                                 varPathSeparator,
                                                                                 hideDataTypes,
                                                                                 typeNameSeparator));
            }
                    ...
                    for (int row = 0; row < tableInfo.getData().size(); row++)
                    {
                        ...
                        if (!(cellValue = tableInfo.getData().get(row)[variableNameIndex].toString()).isEmpty())
                        {
                            ...
                            for (Integer rateIndex : rateIndices)
                            {
                                ...
                                if (!tableInfo.getData().get(row)[rateIndex].toString().isEmpty()
                                    && (rateFilter.isEmpty()
                            ...
                            if (hasRate)
                            {
                                ...
                                for (Integer enumerationIndex : enumerationIndices)
                                {
                                    ...
                                    if (!(cellValue = tableInfo.getData().get(row)[enumerationIndex].toString()).isEmpty())
                                    {
