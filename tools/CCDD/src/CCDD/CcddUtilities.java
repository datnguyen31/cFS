...
public class CcddUtilities
{
    ...
    private static enum HTMLTag
    {
         ...
         *
         * @param htmlTag HTML tag characters
         *
         * @param tagChar HTML tag replacement character for HTML reserved characters
         *****************************************************************************************/
    ...
    protected static <T> T[] concatenateArrays(T[] array1, T[] array2)
    {
        ...
        else
        {
            ...
            concatArray = (T[]) Array.newInstance(array1.getClass().getComponentType(), numRows1 + numRows2);
    ...
    {
        return String.format("<font color=\"#%06x\">" + text + "</font>", (0xffffff & color.getRGB()));
    }
        ...
        if (inputText != null && !inputText.isEmpty())
        {
                    ...
                    switch (inputText.charAt(tagIndex))
                    {
                        ...
                        case '&':
                            ...
                            for (HTMLTag tagInfo : HTMLTag.values())
                            {
                                ...
                                if (tagInfo.isReservedChar() && inputText.startsWith(tagInfo.getHTMLTag(), tagIndex))
                                {
                            ...
                            if (!isFound)
                            {
                                ...
                                // character
                                inputText = inputText.substring(0, tagIndex) + "_" + inputText.substring(tagIndex + 1);
                            }
            ...
            while (inputText.length() > maxLength)
            {
                ...
                else
                {
                    ...
                    // characters
                    padLength = inputText.charAt(index) == ' ' ? " ".length() : HTMLTag.BREAK.getHTMLTag().length();
                }
                ...
                if (inputText.substring(0, index + padLength).contains(HTMLTag.BREAK.getHTMLTag()))
                {
                    ...
                    breakIndex += inputText.substring(0, index + padLength).indexOf(HTMLTag.BREAK.getHTMLTag())
            ...
            for (Tags tag : tags)
            {
                ...
                for (Tags breakTag : tags)
                {
                    ...
                    if (tagIndex > breakTag.getIndex() + (isBreakDone ? tagIndexAdjust : 0))
                    {
