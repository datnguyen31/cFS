...
public class CcddServerPropertyDialog extends CcddDialogHandler
{
    ...
    private void initialize(String errorMessage)
    {
        ...
        switch (dialogType)
        {
            case LOGIN:
                ...
                if (addLoginPanel(selectPnl, gbc, border))
                {
                                          ...
                                          allowResize) == OK_BUTTON)
                    {
                        ...
                        dbControl.openDatabaseInBackground(useActiveDatabase ? dbControl.getProjectName()
                                                                             : DEFAULT_DATABASE);
                    }
                }
            ...
            case DB_SERVER:
                ...
                servers.addAll(Arrays.asList(ccddMain.getProgPrefs().get(SERVER_STRINGS, "").split(STRING_LIST_TEXT_SEPARATOR)));
    ...
    private boolean addLoginPanel(JPanel selectPnl, GridBagConstraints gbc, Border border)
    {
        ...
        else
        {
            ...
            userFld.setBackground(dialogType == ServerPropertyDialogType.LOGIN ? ModifiableColorInfo.INPUT_BACK.getColor()
        ...
        try
        {
            ...
            switch (dialogType)
            {
                ...
                case PASSWORD:
                    ...
                    if (!dbControl.authenticateUser(dbControl.getUser(), String.valueOf(passwordFld.getPassword())))
                    {
                        // Inform the user that the database port field is invalid
                        throw new CCDDException("Password incorrect for user '" + dbControl.getUser() + "'");
                    }
