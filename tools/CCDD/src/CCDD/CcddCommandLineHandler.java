...
public class CcddCommandLineHandler
{
    ...
    private boolean includeTlmSched;
    private boolean includeAppSched;
    ...
    private boolean exportFullDatabase;
    ...
    private class CommandHandler
    {
         ...
         *
         * @throws Exception If a command parameter is invalid
         *****************************************************************************************/
         ...
         *
         * @param parmVal Command parameter
         *
         * @throws Exception If an error occurs executing a command
         *****************************************************************************************/
     ...
     *
     * @param args     Array of command line arguments and associated parameters
     *********************************************************************************************/
    CcddCommandLineHandler(final CcddMain ccddMain, String[] args)
    {
                                        ...
                                        new String[] {"true", "false"})
        {
            ...
            protected void doCommand(Object parmVal)
            {
                ccddMain.getSessionEventLog().setFilter(EventLogMessageType.COMMAND_MSG, (Boolean) parmVal);
            }
        });
                                        ...
                                        new String[] {"true", "false"})
        {
            ...
            protected void doCommand(Object parmVal)
            {
                ccddMain.getSessionEventLog().setFilter(EventLogMessageType.FAIL_MSG, (Boolean) parmVal);
            }
        });
                                        ...
                                        new String[] {"true", "false"})
        {
            ...
            protected void doCommand(Object parmVal)
            {
                ccddMain.getSessionEventLog().setFilter(EventLogMessageType.SERVER_MSG, (Boolean) parmVal);
            }
        });
        ...
        argument.add(new CommandHandler("scriptOutPath",
                                        ...
                                        + "  This command may be used more\n"
                                        + "  than once. This path is in\n"
        ...
        argument.add(new CommandHandler("patch",
                                        "Automatically apply patches to the\n"
                                        + "  database",
                                        "",
                                        CommandLineType.NONE,
                                        CommandLinePriority.PRE_START.getStartPriority())
        ...
        argument.add(new CommandHandler("import",
                                        ...
                                        + "  EDS, JSON, XTCE or C header file",
                                        "'<import sub-commands>'",
                                        CommandLineType.NAME,
        ...
        importArgument.add(new CommandHandler("appendExistingFields",
                                              "Append existing data field(s)\n"
                                              + "  if table exists. Only used\n"
        ...
        importArgument.add(new CommandHandler("useExistingFields",
                                              ...
                                              + "  imported one matches. Only\n"
                                              + "  used if replaceExisting and\n"
                                              ...
                                              new String[] {"true", "false"})
        {
            ...
            protected void doCommand(Object parmVal)
            {
                importFullDatabase = (Boolean) parmVal;
            }
        });
                                        ...
                                        exportArgument)
        {
            ...
            protected void doCommand(Object parmVal) throws Exception
            {
                ...
                String[] separators = new String[] {varPathSeparator, typeNameSeparator,
        ...
        exportArgument.add(new CommandHandler("filePath",
                                              ...
                                              + "  for EDS, XTCE, and for CSV,\n"
                                              + "  JSON if exporting to a single\n"
                                              + "  file). This path is in effect\n"
                                              ...
                                              0)
        {
            ...
            protected void doCommand(Object parmVal)
            {
                // CHeck for the special case where all tables are to be selected
                if (((String) parmVal).equals("all"))
                {
                    tablePaths = new String[] {"all"};
                }
                else
                {
                    // Convert the table path list to the internal format used by script
                    // associations
                    String internalFormat = CcddScriptHandler.convertAssociationMembersFormat((String) parmVal, true);
        ...
        exportArgument.add(new CommandHandler("overwriteFile",
                                              "Overwrite existing file(s). If\n"
                                              + "  the GUI is hidden then any\n"
        ...
        exportArgument.add(new CommandHandler("includeAllInputTypes",
                                              "Include all user-defined input\n"
                                              + "  type definitions (CSV, JSON)",
                                              "true or false (default: false)",
                                              CommandLineType.OPTION,
                                              0,
                                              new Object[] {true, false},
        ...
        exportArgument.add(new CommandHandler("includeBuildInformation",
                                              "Include the CCDD version,\n"
                                              + "  project, host, and user\n"
                                              + "  information",
                                              ...
                                              new String[] {"true", "false"})
        {
            ...
            protected void doCommand(Object parmVal)
            {
                exportFullDatabase = (Boolean) parmVal;
            }
        });

        // Export command - include groups
        exportArgument.add(new CommandHandler("includeGroups",
                                              "Include group definitions and\n"
                                              + "  data fields (CSV, JSON)",
                                              "true or false (default: false)",
                                              CommandLineType.OPTION,
                                              0,
                                              new Object[] {true, false},
                                              ...
                                              new String[] {"true", "false"})
        {
            ...
            protected void doCommand(Object parmVal)
            {
                includeTlmSched = (Boolean) parmVal;
            }
        });
                                        ...
                                        deleteArgument)
        {
            ...
            protected void doCommand(Object parmVal) throws Exception
            {
                ...
                if (!ccddMain.getDbControlHandler().connectToServer())
                {
                    ...
                    {
                        throw new Exception("Delete project failed; project '" + deleteName + "' does not exist");
                    }
                }
                // The attempt to connect to the PostgreSQL server failed
                else
        ...
        deleteArgument.add(new CommandHandler("continueIfMissing",
                                              "Continue to process commands if\n"
        ...
        try
        {
            ...
            for (int priority : priorities)
            {
                ...
                for (int index = 0; index < cmdLnArgs.length; index++)
                {
                    ...
                    {
                        throw new Exception("Unrecognized delimiter for command '" + arg + "'");
                    }
                    ...
                    for (CommandHandler cmd : commandArgument)
                    {
                        ...
                        if (arg.equalsIgnoreCase(cmd.command))
                        {
                            ...
                            if (cmd.hasArgument())
                            {
                                ...
                                {
                                    throw new Exception("Missing argument for command '" + arg + "'");
                                }
                            }
     ...
     *
     * @param exitStatus 0 if all commands executed successfully; 1 if an error occurred
     *********************************************************************************************/
     ...
     *
     * @param arg  Window size command line argument
     *
     * @param parm Window dimensions
     *
     * @param min  Array containing the minimum main window width and minimum main window height
     *
     * @return Main window dimension value (null if an error is found)
     *
     * @throws Exception If the width or height value is not a number or a value is missing
     *********************************************************************************************/
     ...
     *
     * @param arg   Integer value command line argument
     *
     * @param parm  Integer value
     *
     * @param limit Array containing the lower limit value (inclusive) and the upper limit value
     *              (inclusive)
     *
     * @return Integer value (null if an error is found)
     *
     * @throws Exception If the minimum or maximum value is not a number
     *********************************************************************************************/
     ...
     *
     * @param arg  Color command line argument
     *
     * @param parm Color in hexadecimal format or a color name
     *
     * @return Selected color in hexadecimal format (null if an error is found)
     *
     * @throws Exception If the color value is not in the expected hexadecimal format or is not a
     ...
     *
     * @param parm         Command option
     *
     * @param inputOptions Array of valid inputs
     *
     * @param outputValues Array of output values associated with each input option
     *
