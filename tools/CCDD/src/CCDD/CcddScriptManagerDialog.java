 ...
 *
 * \brief Dialog for the user to associate scripts and data tables. The dialog is built on the
...
public class CcddScriptManagerDialog extends CcddFrameHandler
{
            ...
            CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
            {
                ...
                protected void execute()
                {
                    ...
                    dialogPnl.add(new CustomSplitPane(inputsPane,
                                                      scriptHandler.getAssociationsPanel("Script Associations", true,
                                                                                         CcddScriptManagerDialog.this),
                                                      assnSep, JSplitPane.VERTICAL_SPLIT),
                                  gbc);
                        ...
                        public void valueChanged(ListSelectionEvent lse)
                        {
                            ...
                            if (!lse.getValueIsAdjusting() && !isAddingAssn)
                            {
                                ...
                                if (row != -1)
                                {
                                    ...
                                    // field
                                    scriptNameFld.setText(assnsTable.getValueAt(row, assnsTable.convertColumnIndexToView(AssociationsTableColumnInfo.SCRIPT_FILE.ordinal())).toString());
                                    ...
                                    // correctly
                                    String[] tableNames = CcddUtilities.removeHTMLTags(assnsTable.getValueAt(row, assnsTable.convertColumnIndexToView(AssociationsTableColumnInfo.MEMBERS.ordinal()))
                    ...
                    btnUndo.addActionListener(new ValidateCellActionListener()
                    {
                        ...
                        protected void performAction(ActionEvent ae)
                        {
                            ...
                            // Check to see if the command needs to be called again due to this
                            // being a REPLACE command that is being undone
                    ...
                    btnRedo.addActionListener(new ValidateCellActionListener()
                    {
                        ...
                        protected void performAction(ActionEvent ae)
                        {
                            ...
                            // Check to see if the command needs to be called again due to this
                            // being a REPLACE command that is being redone
    ...
    private JPanel createSelectionPanel(String labelText, int tableSelect)
    {
        ...
        if (tableTree.getRowCount() != (tableTree.isRootVisible() ? 1 : 0))
        {
            // Create panels to hold the components of the dialog
     ...
     *                    TableInsertionPoint.SELECTION to insert below the currently selected row,
     *                    or TableInsertionPoint.END to insert as the last row in the table
     *
        ...
        try
        {
            ...
            assnsTable.insertRow(true,
                                 ...
                                 new Object[] {nameFld.getText(),
                                               ...
                                               (scriptFile.exists() ? AvailabilityType.AVAILABLE
                                          ...
                                          List<String> postScriptAssocNames)
    {
        ...
        // Step though each index of the pre-list one time and if a name also appears in the
        // post-list then remove one instance from the post-list
        ...
        // If there are any names left in the post list then step though each name and see if it
        // appears in the pre-list meaning it was a duplicate created by the REPLACE command
