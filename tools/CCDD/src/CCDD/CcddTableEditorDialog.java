...
public class CcddTableEditorDialog extends CcddFrameHandler
{
     ...
     *
     * @param editor           Reference to an existing table editor; null if adding new tables to
     *                         the editor dialog
     *********************************************************************************************/
     ...
     *
     * @param forceUpdate       True to make the changes to other tables (e.g., following a data
     *                          type or macro change); false to only make changes to tables other
     *                          than the one in which the changes originally took place
     *
        ...
        for (CcddTableEditorDialog editorDialog : main.getTableEditorDialogs())
        {
            ...
            for (CcddTableEditorHandler editor : editorDialog.getTableEditors())
            {
                        ...
                        && editor.getTableTypeDefinition().getColumnIndexByInputType(DefaultInputType.MESSAGE_REFERENCE) != -1))
                {
                    ...
                    TableInfo updateInfo = main.getDbTableCommandHandler().loadTableData(editor.getTableInformation().getTablePath(),
                                                                                         true,
                                                                                         true,
                                                                                         false,
                                           editorDialog);
                                           ...
                                           CcddTableEditorHandler editor)
    {
        ...
        // Get the prototype + variable name from that editor owner name by stripping off the root
        // name and separator
        ...
        {
            // Update the variable data types to match the change, if this table represents a
            // structure
            editor.updateDataTypeReferences(oldPrototype, newPrototype);
        }

        // Check if the prototype has changed and the table for the specified editor has the same
        // prototype, or if the variable name has changed and the table for the specified editor is
                ...
                && (protoVarName.equals(oldVariableName) || protoVarName.endsWith("." + oldVariableName))))
        {
            ...
            // Update the tab in the editor dialog for this table with the new name and tool tip
            // text
    ...
    private void initialize(List<TableInfo> tableInformation, CcddTableEditorHandler editor)
    {
        ...
        mntmInsertMacro.addActionListener(new ValidateCellActionListener()
        {
            ...
            protected void performAction(ActionEvent ae)
            {
                // Check if a cell is being edited in the table by checking if the selection start
        ...
        mntmStoreAll.addActionListener(new ValidateCellActionListener()
        {
            ...
            protected void performAction(ActionEvent ae)
            {
                // Only update if one or more of the table's has changes and the user confirms the
                // action
                if (isTablesChanged()
        ...
        mntmManageFields.addActionListener(new ValidateCellActionListener()
        {
            ...
            protected void performAction(ActionEvent ae)
            {
                ...
                activeEditor.getTableInformation().setFieldInformation(activeEditor.getInputFieldPanelHandler().getPanelFieldInformation());
            ...
            protected Object tabMoveCleanup(int oldTabIndex, int newTabIndex, Object tabContents)
            {
                ...
                if (newTabIndex != -1)
                {
                    ...
                    tableEditors.add(newTabIndex
                                     - (oldTabIndex != -1 && newTabIndex > oldTabIndex ? 1 : 0),
                                     editor);
                }
     ...
     *
     * @param tableEditor Reference to the table editor for which the change indicator is to be
     *                    updated
     *********************************************************************************************/
