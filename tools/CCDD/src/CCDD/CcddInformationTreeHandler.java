...
public abstract class CcddInformationTreeHandler extends CcddCommonTreeHandler
{
                               ...
                               Component parent)
    {
        ...
        if (infoDefinitions != null)
        {
            ...
            infoTreeModel = undoHandler != null ? undoHandler.new UndoableTreeModel(root)
            {
                ...
                {
                    CcddInformationTreeHandler.this.nodeRenameCleanup(wasValue, isValue);
                }
     ...
     * @param parent          GUI component over which to center any error dialog
     *********************************************************************************************/
    CcddInformationTreeHandler(CcddMain ccddMain, List<String[]> infoDefinitions, Component parent)
    {
                ...
                if (!infoNodes.contains(infoNode))
                {
                    ...
                    for (Object[] sourcePath : sourcePaths)
                    {
                        ...
                        if (onlyIfPrimitive)
                        {
                            ...
                            if (lastPeriod != -1
                                && dataTypeHandler.isPrimitive(removeExtraText(lastNode.substring(0, lastPeriod))))
                            {
